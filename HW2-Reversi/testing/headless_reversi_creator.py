...
class HeadlessReversiCreator(object):
    ...
    def __init__(self, player1, player1_color, player2, player2_color, b_size=8):
        """
        ...
        :param b_size: Board will have size [boardSize x boardSize]
        """
    ...
    def play_game(self):
        ...
        while self.board.can_play(self.current_player_color):
            ...
            if move is None:
                print('Player %d returns None instead of a valid move. Move takes %.3f ms.' % (
...
if __name__ == "__main__":
    ...
    players = []
    # if only one player specified, it will play against itself
    for i in [0, -1]:
        try:
            to_import = args[i]
            if ".py" in args[i]:

...
class ReversiCreator(object):
    ...
    def get_player_move(self, q):
        """
        Returns current player move to Queue, meant for threaded call. Now deprecated.
        :param q: Queue for returning the move with spent time.
        """
        start_time = time.time()
        move = self.current_player.move(self.board.get_board_copy())
        endTime = time.time()
        moveTime = (endTime - start_time) * 1000  # to display in millisecond
    ...
    def play_game(self, interactivePlayerColor=-1):
        ...
        """
        print('play game with interactive %d' % interactivePlayerColor)
        ...
        while self.board.can_play(self.current_player_color) and not self.paused:
            ...
            if move is None:
                ...
                inform_str = 'Player %d return None instead of a valid move.' % self.current_player_color
                self.gui.inform(inform_str, 'red')
        ...
        current_player_opponent_color = self.get_opponent_player_color(self.current_player_color)
        if next_player_id == -1 and self.board.can_play(current_player_opponent_color):
            print("opponent player can play an current can not")
            self.change_player()
            next_player_id = current_player_opponent_color

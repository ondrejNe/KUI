...
class ReversiView:
    ...
    def __init__(self, players, boardSize=8, w=850, h=410):
        ...
        self.gridw = 410
        self.gridh = 410
        self.gridspacing = int((self.gridh - 2 * self.offy) / self.boardSize)  # quick and dirty
    ...
    def draw_stone(self, x, y, color):
        ...
        :param color: 0 for blue, 1 for red
        """
        ...
        self.stone_board[x][y] = self.grid.create_oval(x_coord, y_coord, x_coord + diameter, y_coord + diameter,
    ...
    def draw_game_info_grid(self):
        ...
        scale = Scale(self.info, variable=self.scale_var, command=self.sleep_time_change_handler, from_=0, to=1000,
                      resolution=10, width="15", orient=HORIZONTAL, length="225")
        scale.set(200)
        ...
        for i in range(2):
            ...
            self.label_player_max_time[i] = Label(self.info, text="%.2f [ms]" % 0.0, font=("Helvetica", 10),
            ...
            self.labels_player_name[i] = Label(self.info, text="Player%d:" % (i), font=("Helvetica", 12),
                                               foreground=self.colors[i])
    ...
    def draw_game_grid(self):
        ...
        for x in range(0, gridsize):
            for y in range(0, gridsize):
                ...
                self.grid.create_text(offx + (spacing * x) + spacing / 2, offy + (spacing * y) + spacing / 2,
                                      text=arrayText)
        # line rows
            ...
            for i in range(2):
                ...
                if self.option_menus_vars[i].get() == self.interractivePlayerName:
                    ...
                    if i == 0:
                        ...
                        self.game.player1 = player_creator.create_player(player_class, self.game.player1_color,
                                                                         self.game.player2_color, self.boardSize)

                    else:
                        player_class = self.possiblePlayers[self.option_menus_vars[i].get()]
                        self.game.player2 = player_creator.create_player(player_class, self.game.player2_color,
                                                                         self.game.player1_color, self.boardSize)
    ...
    def print_move_max_times(self, maxTimesMS):
        ...
        :param maxTimeMS: array of max time needed for move.
        '''
    ...
    def place_stone_click_handler(self, event):
        ...
        if self.game_state != GameState.STOPPED and len(
                self.interactive_player_ids) >= 1 and self.game.current_player_color in self.interactive_player_ids:

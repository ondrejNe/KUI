...
from kuimaze2.rendering import ValueCanvas, QValueCanvas
...
class MDP:
    ...
    def __init__(
       ...
       """
        self._map = map
        self._actions_model = DeterministicActions() if not action_probs else StochasticActions(**action_probs)
    ...
    def _get_transition_result(self, state: State, action: Action) -> State:
    ...
    def get_next_states_and_probs(self, state: State, action: Action) -> list[tuple[State, float]]:
        ...
        return [(self._get_transition_result(state, action), prob) for action, prob in actions_probs.items()]
...
class MDPProblem(MDP):
    ...
    @cached_property
    def _rewards_range(self) -> tuple[float, float]:
        return self._rewards.danger, self._rewards.goal
...
class TkMDPView(MDPView):
    ...
    def __init__(self, env: MDPProblem):
        ...
        self.v_canvas = ValueCanvas(self.tk, map=self.env._map, value_range=self.env._rewards_range)
        self.v_canvas.pack()
        self.q_canvas = QValueCanvas(self.tk, map=self.env._map, value_range=self.env._rewards_range)
        self.q_canvas.pack()

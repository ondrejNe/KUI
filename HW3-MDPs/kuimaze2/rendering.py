...
class SquareCanvas(MapCanvas):
    ...
    def draw_path(self, path: list[State]):
        ...
        self.path_line = self.create_line(points, fill="red", width=7, capstyle="round", arrow=tk.LAST,
                                          arrowshape=(12, 20, 6))
    ...
    def draw_triangle_texts(self, texts=None, default_text=""):
        ...
        for sq in self.map:
            ...
            for action in Action:
                ...
                self.triangle_texts[(sq.position, action)] = self.create_text(left, top, text=text, font=(FONT_FAMILY, round(self.font_size*0.6)))
...
class ValueCanvas(SquareCanvas):
    ...
    def set_square_colors_from_values(self, values: dict[State, float]):
        colors = {position: self.color_from_value(value) for position, value in values.items()}

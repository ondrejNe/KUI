...
def walk_randomly(env):
    # If you want the same outcomes in repeated runs, set the random generator seed
    # gym.spaces.prng.seen(number) # For older aigym versions
    # env.action_space.seed(number) # For newer aigym versions
    obv = env.reset()
    state = obv[0:2]
    total_reward = 0
    is_done = False

...
import kuimaze
from kuimaze.types import State, PathSection, ensure_state
from kuimaze import ACTION
from .map_generator import maze as mapgen_maze

EIGHT_DIRS = (
    False  # can robot/agent go in 8 directions? If not, 4 neighbourhood is considered
)
...
class MazeEnv(gym.Env):
    ...
    def __init__(
        ...
        self.observation_space = spaces.Tuple(
            (spaces.Discrete(self._xsize), spaces.Discrete(self._ysize))
        )
        self.seed()
        self.reset()
    ...
    def step(self, action):
        ...
        # reward = self._problem.get_reward(self._curr_state)
    ...
    def save_path(self):
        ...
        assert (
            len(self._path) > 0
        ), "Path length must be greater than 0, for easy enviroment call set_path first"
        # at the moment it assumes the output directory exists
        pathfname = os.path.join(
            os.path.dirname(os.path.dirname(sys.argv[0])), "saved_path.txt"
        )
        with open(pathfname, "wt") as f:
            # then go backwards throught the path restored by bactracking
            if (
                type(self._path[0]) == tuple or type(self._path[0]) == list
            ) and not self._gym_compatible:
                for pos in self._path:
                    f.write(
                        "x:{}, y:{}, z:{}\n".format(
                            pos[0], pos[1], self._get_depth(State(pos[0], pos[1]))
                        )
                    )
            if self._gym_compatible:
                for pos in self._path:
        ...
        if self._informed:
            ret = [
                (
                    self._curr_state.x,
                    self._curr_state.y,
                    self._get_depth(self._curr_state),
                )
            ]
    ...
    def _get_depth(self, state):
        ...
        @param state: namedtuple kuimaze.State
        @return: float
        """
...
class EnvAgent(kuimaze.BaseAgent):
    ...
    def find_path(self):
        ...
        for i in range(len(self.__path) - 1):
            ret.append(PathSection(self.__path[i], self.__path[i + 1], 1, None))
        try:
...
class EasyMazeEnv(MazeEnv):
    ...
    def set_path(self, path):
        ...
        self._path = path
        if self._gui_on:
            assert (type(path[0]) == list or type(path[0]) == tuple) and (
                len(path[0]) == 2 or len(path[0]) == 3
            )
            previus_state = None
            for state_list in path:
                if previus_state != None:
                    if (
                        abs(state_list[0] - previus_state[0]) > 1
                        and abs(state_list[1] - previus_state[1]) > 1
                    ):
                        raise AssertionError(
                            "The path is not continuous - distance between neighbouring path segments should be max. 2 ** (1/2)"
                        )
    ...
    def _is_available(self, new_state):
        ...
        tmp.extend(
            [
                self._problem.result(self._curr_state, 0),
                self._problem.result(self._curr_state, 1),
                self._problem.result(self._curr_state, 2),
                self._problem.result(self._curr_state, 3),
                self._problem.result(self._curr_state, 4),
                self._problem.result(self._curr_state, 5),
                self._problem.result(self._curr_state, 6),
                self._problem.result(self._curr_state, 6),
            ]
        )
    ...
    def _get_cost(self, curr, last):
        ...
        if curr != last:
            reward = (
                pow(pow(vector[0], 2) + pow(vector[1], 2), 1 / 2)
                + z_axis
                + addition_cost
            )
        return reward
    ...
    def expand(self, position):
        ...
        if EIGHT_DIRS:
            tmp = [
                self._problem.result(maze_pose, 0),
                self._problem.result(maze_pose, 1),
                self._problem.result(maze_pose, 2),
                self._problem.result(maze_pose, 3),
                self._problem.result(maze_pose, 4),
                self._problem.result(maze_pose, 5),
                self._problem.result(maze_pose, 6),
                self._problem.result(maze_pose, 7),
            ]
        else:
            tmp = [
                self._problem.result(maze_pose, 0),
...
class MDPMaze(MazeEnv):
    ...
    def __init__(self, map_image=None, grad=(0, 0), probs=None, node_rewards=None):
        if probs is not None:
            super().__init__(
                informed=False, gym_compatible=True, deter=False, map_image_dir=map_image, grad=grad, node_rewards=node_rewards
            )
            self._problem.set_probs_table(probs[0], probs[1], probs[2], probs[3])  # set probabilities here
        else:
            super().__init__(informed=False, gym_compatible=True, deter=True, map_image_dir=map_image, grad=grad, node_rewards=node_rewards)
            self._problem.set_probs_table(1, 0, 0, 0)  # set probabilities here
    ...
    def is_terminal_state(self, state):
        ...
        return self._problem.is_goal_state(state) or self._problem.is_danger_state(
            state
        )
    ...
    def get_reward(self, state1=None, action=None, state2=None):
        """Return the reward for state1-action-state2 transition."""
        assert any(
            [state1, action, state2]
        ), "At least one of state1, action, state2 must be provided."
        # The kuimaze.Maze assumes that reward is just a function of state1.
        state1 = ensure_state(state1)
        return self._problem.get_state_reward(state1)
...
class HardMaze(MazeEnv):
    ...
    def __init__(self, map_image=None, grad=(0, 0), probs=None, node_rewards=None):
        if probs is not None:
            super(HardMaze, self).__init__(
                False, True, False, map_image, grad, node_rewards=node_rewards
            )
            self._problem.set_probs(
    ...
    def step(self, action):
        ...
        assert not self._episode_finished, "Episode finished, no more actions possible. Call reset() to start a new episode."
    ...
    def _get_reward(self, curr, last):
        ...
        if curr != last:
            reward = (
                reward - z_axis
            )  # more or less depending on the gradient; going up costs more
        if self._problem.is_goal_state(curr):
            ...
            if self._gym_compatible:
                self._player.set_path(self._path)
                self._player.find_path()
        return reward, self._episode_finished
